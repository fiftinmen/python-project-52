...
class _TestStatusesUtilsMixin(_TestUtilsMixin):
    ...
    def delete_statuses(self, statuses=None):
        if statuses is None:
            statuses = (
...
class TestsStatuses(TestCase, _TestStatusesUtilsMixin):
    def setUp(self):
        ...
        self._tests_to_fail_tuple = (
            ...
            (
                "_test_statuses_delete_with_related_task_post_fail",
                valid_statuses,
            ),
        )
    ...
    def _test_statuses_delete_with_related_task_post_fail(self, data):
        user = self.default_user
        self.client.force_login(user)
        status = self.create_status(data)
        status.task_set.create(name=f'{data["name"]}_task', author=user)
        response = self.client.post(
            reverse_lazy("statuses_delete", args=(status.pk,))
        )
        self.assertEqual(response.status_code, 302)
